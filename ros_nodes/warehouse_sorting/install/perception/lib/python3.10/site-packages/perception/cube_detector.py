...
class RealSensePCSubscriber(Node):
    def __init__(self):
        ...
        # A: 0.045813724545475915, B: 0.9766019794425214, C: 0.21011824383479893, D: -0.1145323726940536
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        candidate_pts = points[max_dist_mask]
        candidate_clrs = colors[max_dist_mask]

        xz = candidate_pts[:,[0,2]]
        y = candidate_pts[:,1]
        ...
        table_points = candidate_pts[inlier_mask]
        object_points = candidate_pts[~inlier_mask]

        y_pred = ransac.predict(xz)
        diff = y - y_pred
        plane_mask = (diff < -0.02) & (diff > -0.5) # between 1cm and 30cm above table
        filtered_points = candidate_pts[plane_mask]
        filtered_colors = candidate_clrs[plane_mask]
        # filtered_points = candidate_pts[~inlier_mask]
        # filtered_colors = candidate_clrs[~inlier_mask]
        ...
        for label in range(max_label + 1):
            ...
            if len(indices) > MAX_CLUSTER_POINTS:
                if cube_label == "blue":
                    ...
                    self.get_logger().info(f"Is your obstacle {cube_label}?")
