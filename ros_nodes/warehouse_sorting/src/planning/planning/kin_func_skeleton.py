# Replace with your own kin_func_skeleton.py from HW3
...
def skew_3d(omega):
    ...
    omega_hat = np.array([[0.0, -omega[2], omega[1]],
                          [omega[2], 0.0, -omega[0]],
                          [-omega[1], omega[0], 0.0]], dtype=float)
    return omega_hat
...
def rotation_3d(omega, theta):
    ...
    return (np.eye(3) + (w_hat/w_norm) * np.sin(w_norm*theta) + ((w_hat @ w_hat )/(w_norm**2)) * (1 - np.cos(w_norm*theta)))
...
def hat_3d(xi):
    ...
    v = xi[0:3]
    omega = xi[3:6]
    xi_hat = np.array([[0, -omega[2], omega[1], v[0]],
...
def homog_3d(xi, theta):
    ...
    if np.linalg.norm(omega) == 0:
        ...
        w_norm = np.linalg.norm(omega)
        R = (np.eye(3)
            + (np.sin(w_norm*theta)/w_norm) * w_hat
            + ((1 - np.cos(w_norm*theta)) / (w_norm**2)) * (w_hat @ w_hat))

        V = (np.eye(3) * theta
            + ((1 - np.cos(w_norm*theta)) / (w_norm**2)) * w_hat
            + ((w_norm*theta - np.sin(w_norm*theta)) / (w_norm**3)) * (w_hat @ w_hat))
...
def prod_exp(xi, theta):
    ...
    for i in range(xi.shape[1]):
        exp_m = exp_m @ homog_3d(xi[:, i], theta[i])
    return exp_m
