...
class UR7e_CubeGrasp(Node):
    def __init__(self):
        ...
        self.cube_pub = self.create_subscription(PointStamped, '/cube_in_base_link', self.cube_callback, 1) # TODO: CHECK IF TOPIC ALIGNS WITH YOURS
    ...
    def cube_callback(self, cube_pose):
        ...
        '''
        x_pre = cube_pose.point.x - 0.09
        y_pre = cube_pose.point.y - 0.05
        z_pre = cube_pose.point.z + 0.185
        ik_result = self.ik_planner.compute_ik(current, x_pre, y_pre, z_pre)
        ...
        '''
        x_grasp = cube_pose.point.x - 0.09
        y_grasp = cube_pose.point.y - 0.05
        z_grasp = cube_pose.point.z + 0.16
        ik_result = self.ik_planner.compute_ik(current, x_grasp, y_grasp, z_grasp)
        self.job_queue.append(ik_result)
        current = ik_result
        ...
        y_release = y_pre
        z_release = z_pre
        ik_result = self.ik_planner.compute_ik(current, x_release, y_release, z_release)
        self.job_queue.append(ik_result)
        current = ik_result
